...
import { documentApi } from '@/features/documents/document.api';
...
export function DocumentsTab({ studentId }: DocumentsTabProps) {
  ...
  const { data: documents, isLoading } = useQuery({
    ...
    queryFn: () => documentApi.listForStudent(studentId),
  });

  const uploadMutation = useMutation({
    mutationFn: (file: File) => documentApi.upload(studentId, file, category),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['documents', studentId] }),
  });
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col divide-y divide-border rounded-lg border border-border">
        {documents?.map((doc) => (
          <div key={doc._id} className="flex items-center justify-between px-4 py-3 text-sm">
            <div>
