...
function TemplateForm({ template, studentId }: { template: FormTemplate; studentId: string }) {
  ...
  const existing = submissions?.find((s) => s.template._id === template._id);
  ...
  const submitMutation = useMutation({
    mutationFn: () => formApi.submit(template._id, studentId, values),
  ...
  return (
    <div className="flex flex-col gap-4">
      {templates.map((template) => (
        <TemplateForm key={template._id} template={template} studentId={studentId} />
      ))}
    </div>
  );
}
