...
export function ChecklistTab({ studentId, destinationCountry }: ChecklistTabProps) {
  ...
  useEffect(() => {
    ...
    const matches = templates.filter((t) => !assignedTemplateIds.has(t._id));
  ...
  const availableTemplates = destinationCountry ? [] : (templates ?? []).filter((t) => !assignedTemplateIds.has(t._id));
                ...
                .map((item) => {
                  ...
                  return (
                    <button
                      key={item.key}
                      onClick={() =>
                        itemMutation.mutate({ progressId: progress._id, itemKey: item.key, completed: !completed })
                      }
