...
export function CreateWorkflowModal({ open, onOpenChange }: CreateWorkflowModalProps) {
  ...
  const {
    ...
    defaultValues: { name: '', country: '', steps: [{ title: '' }] },
  });
  const { fields, append, remove } = useFieldArray({ control, name: 'steps' });

  const createMutation = useMutation({
    mutationFn: (values: FormValues) => {
      const keys = uniqueSlugs(values.steps.map((s) => s.title));
      return workflowApi.createTemplate({
        name: values.name,
        country: values.country || undefined,
        steps: values.steps.map((s, i) => ({ key: keys[i], title: s.title, order: i })),
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workflow-templates'] });
      reset({ name: '', country: '', steps: [{ title: '' }] });
      onOpenChange(false);
    },
  });
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 max-h-[85vh] w-full max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-lg border border-border bg-card p-6 shadow-lg">
          ...
          <form onSubmit={handleSubmit((values) => createMutation.mutate(values))} className="flex flex-col gap-4">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-card-foreground">Quốc gia (tùy chọn)</label>
              <select
                {...register('country')}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring"
              >
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                <label className="text-sm font-medium text-card-foreground">Các bước</label>
                <button
                  type="button"
              ...
              {errors.steps && <p className="mt-1 text-xs text-red-500">{errors.steps.message}</p>}
            </div>
