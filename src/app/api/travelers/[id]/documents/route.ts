...
export const GET = withErrorHandling(async (_req, { params }: { params: Promise<{ id: string }> }) => {
  ...
  await connectDB();
  const docs = await TravelerDocument.find({ travelerId })
    .select('-data')
    .sort({ category: 1, version: -1 });
  return ok(docs.map((d) => toTravelerDocumentDTO(d.toObject())));
});
...
export const POST = withErrorHandling(async (req, { params }: { params: Promise<{ id: string }> }) => {
  ...
  const latest = await TravelerDocument.findOne({ travelerId, category }).sort({ version: -1 });

  const ext = file.name.includes('.') ? file.name.slice(file.name.lastIndexOf('.')) : '';
  const data = Buffer.from(await file.arrayBuffer());

  const doc = await TravelerDocument.create({
    travelerId,
    category,
  ...
  return ok(toTravelerDocumentDTO(obj), 201);
});
