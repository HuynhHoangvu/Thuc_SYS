...
export function WorkflowTab({ studentId, destinationCountry }: WorkflowTabProps) {
  ...
  useEffect(() => {
    ...
    const matches = templates.filter((t) => t.country === destinationCountry && !assignedTemplateIds.has(t.id));
  ...
  const availableTemplates = (templates ?? []).filter(
    (t) => !assignedTemplateIds.has(t.id) && t.country !== destinationCountry
  );
                ...
                .map((step) => {
                  ...
                  return (
                    <button
                      key={step.key}
                      onClick={() =>
                        stepMutation.mutate({ progressId: progress.id, stepKey: step.key, completed: !completed })
                      }
