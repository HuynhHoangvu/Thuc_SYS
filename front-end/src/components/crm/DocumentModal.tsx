...
import DocumentFilterBar from "../shared/DocumentFilterBar";
...
const DocumentModal: React.FC<DocumentModalProps> = ({ show, onClose, taskId, task }) => {
  ...
  const [checklistType, setChecklistType] = useState("study");
  ...
  const handleDownloadFile = async (fileUrl: string, fileName: string) => {
    try {
      const fullUrl = fileUrl.startsWith("http")
        ? `${API_URL}/api/tasks/download-proxy?url=${encodeURIComponent(fileUrl)}&name=${encodeURIComponent(fileName)}`
        : `${API_URL}${fileUrl}`;
  ...
  const allUploadedFiles = useMemo(() => {
    const list: { reqId: string; reqName: string; file: SavedFile; index: number }[] = [];
    Object.entries(uploadedFiles).forEach(([reqId, files]) => {
      if (!files || files.length === 0) return;
      
      let reqName = reqId;
      if (reqId.startsWith("extra_")) {
        reqName = reqId.replace("extra_", "").replace(/_/g, " ");
      } else {
      ...
      files.forEach((file, index) => {
        list.push({ reqId, reqName, file, index });
      });
    });
    return list;
  }, [uploadedFiles, baseRequirements]);
      ...
      Object.entries(uploadedFiles).forEach(([reqId, files]) => {
        ...
        files.forEach((file) => {
          const fullUrl = file.url.startsWith("http")
            ? `${API_URL}/api/tasks/download-proxy?url=${encodeURIComponent(file.url)}`
            : `${API_URL}${file.url}`;
  ...
  return (
    <Modal show={show} onClose={onClose} size="6xl" dismissible>
      {/* ── Header ── */}
      <div className="p-5 border-b border-gray-200 bg-blue-50/50 rounded-t-lg flex justify-between items-center gap-4 chinese-modal-header">
      ...
      {/* ── Kho tệp hồ sơ đã nộp (All-in-one File Gallery) ── */}
      {allUploadedFiles.length > 0 && (
        <div className="mx-6 mt-2 mb-4 p-4 bg-emerald-50/30 border border-emerald-100 rounded-xl">
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-xs font-bold text-emerald-800 uppercase tracking-wider flex items-center gap-1.5">
              <span>📂</span> Kho tệp hồ sơ đã nộp ({allUploadedFiles.length} tệp)
            </h4>
            <span className="text-[10px] text-emerald-600 italic">Click biểu tượng tải để tải về · Di chuột để xem nhanh ảnh</span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2.5 max-h-36 overflow-y-auto custom-scrollbar pr-1">
            {allUploadedFiles.map(({ reqId, reqName, file, index }) => {
              const isImg = isImage(file.name);
              return (
                <div 
                  key={`${reqId}-${index}`} 
                  className="relative group bg-white border border-gray-200 hover:border-emerald-400 hover:shadow-sm p-2 rounded-lg flex flex-col justify-between transition-all gap-1.5 min-w-0"
                >
                  {/* File icon and representation */}
                  <div className="flex items-start gap-1.5 min-w-0">
                    <div className="shrink-0 mt-0.5">
                      <FileTypeIcon name={file.name} className="w-4 h-4" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-[11px] font-bold text-gray-700 truncate" title={file.name}>
                        {file.name}
                      </p>
                      <p className="text-[9px] text-gray-400 truncate" title={reqName}>
                        {reqName}
                      </p>
                    </div>
                  </div>
                  ...
                  {isImg && (
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:flex z-[9999] pointer-events-none">
                      <div className="bg-white rounded-xl shadow-2xl border border-gray-200 p-1.5">
                        <img
                          src={file.url}
                          alt={file.name}
                  ...
                  <div className="flex items-center justify-end gap-1.5 pt-1.5 border-t border-gray-100">
                    ...
                    <button
                      onClick={() => handleRemoveFile(reqId, index)}
                      className="p-1 text-red-500 hover:text-red-700 hover:bg-red-50 rounded transition-colors cursor-pointer font-bold text-xs leading-none"
                      title="Xóa tệp"
                    >
                      ×
                    </button>
