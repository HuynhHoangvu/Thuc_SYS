...
const ActivityListModal: React.FC<ActivityListModalProps> = ({
  ...
  const parsedActivities = activities.map(act => {
    ...
    if (!act.completed && act.dueText) {
      if (act.dueText === "Chưa có hạn") {
        computedStatus = "Đã lên kế hoạch";
      } else {
        const match = act.dueText.match(/(\d{1,2})\/(\d{1,2})\/(\d{4})/);
        if (match) {
    ...
    return { ...act, computedStatus };
  });

  const overdueActivities = parsedActivities.filter((a) => a.computedStatus === "Quá hạn" && !a.completed);
  const todayActivities = parsedActivities.filter((a) => a.computedStatus === "Hôm nay" && !a.completed);
  const plannedActivities = parsedActivities.filter(
    (a) => a.computedStatus === "Đã lên kế hoạch" && !a.completed
  );
  ...
  return (
    ...
    >
      <div className="bg-white rounded-lg flex flex-col max-h-[85vh] overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 chinese-modal-header">
        ...
        <div className="overflow-y-auto custom-scrollbar flex-1">
          {overdueActivities.length > 0 && (
            <div>
              <div className="flex justify-between items-center px-4 py-2 bg-red-50 border-b border-red-200">
              ...
              <div className="divide-y divide-gray-100">
                {overdueActivities.map(renderActivityItem)}
              </div>
            </div>
          )}
          ...
          {overdueActivities.length === 0 && todayActivities.length === 0 && plannedActivities.length === 0 && (
