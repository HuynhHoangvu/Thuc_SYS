...
import { createTaskService, updateTaskService, deleteTaskService, moveTaskService, moveProcessingTaskService, sendNotificationService, } from "../services/task.service.js";
...
export const moveTask = asyncHandler(async (req, res) => {
    try {
        ...
        const updatedTask = await moveTaskService(id, columnId);
        getIO().emit("data_changed");
        res.status(200).json(updatedTask);
    }
    catch (error) {
...
export const moveProcessingTask = asyncHandler(async (req, res) => {
    try {
        const { id } = req.params;
        const { processingColId } = req.body;
        const updatedTask = await moveProcessingTaskService(id, processingColId);
        getIO().emit("data_changed");
