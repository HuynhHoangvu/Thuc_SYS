...
export const createFolderService = async (name, parentId) => {
...
export const moveFolderService = async (id, parentId) => {
    if (id === parentId)
...
export const renameFileService = async (id, name) => {
    if (!name)
        throw new Error("File name must not be empty");
    return prisma.docFile.update({ where: { id }, data: { name } });
};
export const moveFileService = async (id, folderId) => {
